/**
 * Verizon Connect Webhook Ingestion Endpoint
 *
 * POST /api/webhooks/verizonconnect
 *
 * Handles incoming Verizon Connect (Reveal / Fleetmatics) webhook pushes.
 *
 * Flow:
 * 1. Read raw body bytes
 * 2. Verify HMAC-SHA256 signature via X-Signature header
...
import { verifyWebhookSignature, normalizeWebhook } from '../telematics/lib/providers/verizonconnect.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 2. Find connection
    const vehicleId = payload.data?.vehicleId || payload.data?.vehicle?.id || null;
    ...
    if (!userId) {
      console.warn('Verizon Connect webhook received but no matching connection found');
    ...
    console.error('Verizon Connect webhook error:', err);
