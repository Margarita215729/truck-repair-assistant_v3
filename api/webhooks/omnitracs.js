/**
 * Omnitracs Webhook Ingestion Endpoint
 *
 * POST /api/webhooks/omnitracs
 *
 * Handles incoming Omnitracs webhook pushes.
 *
 * Flow:
 * 1. Read raw body bytes
 * 2. Verify HMAC-SHA256 signature via X-Omnitracs-Signature header
...
import { verifyWebhookSignature, normalizeWebhook } from '../telematics/lib/providers/omnitracs.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!userId) {
      console.warn('Omnitracs webhook received but no matching connection found');
    ...
    console.error('Omnitracs webhook error:', err);
