...
export interface ErrorCode {
  code: string;
  description: string;
  category: 'engine' | 'transmission' | 'brakes' | 'electrical' | 'emissions' | 'safety' | 'body' | 'general' | 'fuel' | 'cooling' | 'air' | 'suspension';
...
export const getModelsForManufacturer = (manufacturerName: string) => {
  const manufacturer = truckManufacturers.find(m => m.name === manufacturerName);
  return manufacturer ? manufacturer.models : [];
};

export const getErrorCodesForManufacturer = (manufacturerName: string) => {
  const manufacturer = truckManufacturers.find(m => m.name === manufacturerName);
  return manufacturer ? manufacturer.errorCodes : [];
};

export const getErrorCodesForModel = (manufacturerName: string, modelId: string) => {
  const manufacturer = truckManufacturers.find(m => m.name === manufacturerName);
